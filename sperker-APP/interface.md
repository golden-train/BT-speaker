...
| 查电量 | `{"cmd":"getBattery"}` | P7 电源管理；响应结构见 §4.2（实现前回 `not_implemented`） |

软件可用"发送后收到 `not_implemented`"探测功能是否可用，无需握手。
### 调试中心命令（协议已定，App 已实现；固件实现后生效）

| 命令 | 请求 | 响应 | 说明 |
|---|---|---|---|
| 读调试配置 | `{"cmd":"getConfig"}` | `{"ok":true,"cmd":"getConfig","config":{...}}` | 返回声道增益/平衡/自定义 EQ，见 §4.1 |
| 声道增益 | `{"cmd":"setChannelGain","channel":"left","gain":90}` | `{"ok":true,"cmd":"setChannelGain"}` | `channel`=`left`/`right`；`gain` 0–100（%），100=基准 |
| 立体声平衡 | `{"cmd":"setBalance","balance":-30}` | `{"ok":true,"cmd":"setBalance"}` | `balance` -100–100；负=左强，正=右强，0=居中 |
| 自定义 EQ | `{"cmd":"setCustomEq","freq":1000,"gain":3}` | `{"ok":true,"cmd":"setCustomEq"}` | `freq`=60/250/1000/4000/12000 Hz；`gain` -12–+12 dB |

> 说明：以上属「双音箱微调 + 自定义音调」调试中心功能。App 端已完整实现，可用「模拟连接」脱离真机测试；固件当前返回 `not_implemented`。实现路径见 §9（`audio_service` 增加 L/R 独立增益与数字 EQ，`settings` 持久化到 NVS）。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `channelGain.left` / `channelGain.right` | int | 0–100（%）；100=基准，用于双音箱音量微调 |
| `balance` | int | -100–100；负=左强，正=右强，0=居中 |
| `customEq[]` | array | 5 段自定义 EQ；`freq` 单位 Hz，`gain` 单位 dB（-12–+12） |
...
> 说明：`battery` 事件已在协议中定义但**当前不发出**（P7 实现）。实现方案与固件改动清单见 `bt-speaker/docs/battery-implementation.md`。

### 4.2 `getBattery` 响应结构（P7 实现后）
...
| 模块 | 头文件 | 对外能力 |
|---|---|---|
| 音频服务 | `src/audio/audio_service.h` | `AudioService audio`：`init/setVolume/getVolume/play/pause/toggle/next/prev/getPlayState/isBtConnected/getTitle/getArtist`；调试中心命令待实现（L/R 独立增益、平衡、5 段数字 EQ） |
| 事件总线 | `src/core/events.h` | `EventBus events`：`begin/publish/addListener/dispatch`（解耦音频→显示/控制） |
| 偏好存储 | `src/core/settings.h` | `Settings::init/getVolume/setVolume/getEq/setEq/getSource/setSource`（NVS 持久化）；调试配置（声道增益/平衡/自定义 EQ）待加入 |
